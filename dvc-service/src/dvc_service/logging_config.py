"""Configuration logging — DS_COVID DVC Service.

Logs JSON structurés sur stdout + fichier rotatif tmp/logs/dvc-service.log
(si accessible) + envoi asynchrone non-bloquant au log-service central (si
disponible).

Dupliqué depuis data-service/src/data_service/logging_config.py (même
raison : autonomie de build de chaque service, cf. R8/frontières de service
du projet — pas d'import Python cross-service même pour un module utilitaire).
"""
...
def _configure_root(service_name: str) -> None:
...
def setup_logging(log_dir: Path | None = None) -> None:
    """Configure le logging dvc-service (paramètre log_dir non utilisé, hérité)."""
    _configure_root(service_name="dvc-service")
