...
`backend/src/ds_covid/preprocessing.py` et `scripts/preprocess.py`) plutôt que d'en
...
# backend/src est copié dans l'image Docker à côté de app/ (cf. infrastructure/docker/
# backend/Dockerfile) pour permettre cette réutilisation du pipeline d'entraînement.
