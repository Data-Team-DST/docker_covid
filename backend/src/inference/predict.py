...
Modification Phase 4 : chaque branche (succès, erreur 500) alimente
maintenant les métriques Prometheus définies dans app.api.metrics, en plus
du comportement existant (logs + stats.increment_predict d'origine, qui
est remplacé par record_prediction — voir la docstring de metrics.py pour
la justification de la rétrocompatibilité des noms de métriques).
"""
...
from app.api.metrics import record_prediction, record_prediction_error
...
async def predict(
    ...
    **Authentification** : header `X-API-Key` obligatoire.
    ...
    if not model_loader.is_loaded:
        record_prediction_error("model_not_loaded")
        raise HTTPException(
            status_code=503,
            detail=(
    ...
    if file.content_type not in ("image/jpeg", "image/png"):
        record_prediction_error("invalid_format")
        raise HTTPException(
    ...
    try:
        ...
        record_prediction(predicted_class, confidence, latency_ms)
        logger.info(
            "Prédiction : %s (%.1f%%) | %sms",
            predicted_class,
            confidence * 100,
        ...
        record_prediction_error(type(e).__name__)
