...
def load_config_files(project_root: Path, environment: str) -> dict:
    ...
    try:
        ...
        print(
            f"⚠️ Fichier {default_config_path} non trouvé, utilisation config minimale"
        )
        config_data = {}
...
def build_config(project_root: Path, environment: str) -> Config:
    ...
    data_dir = (
        project_root / data_dir_str
        if not Path(data_dir_str).is_absolute()
        else Path(data_dir_str)
    )
    ...
    config_kwargs = {
        ...
        "freeze_base_layers": flat_config.get(
            "models_transfer_learning_freeze_base_layers", True
        ),
        "fine_tune_layers": flat_config.get(
            "models_transfer_learning_fine_tune_layers", 10
        ),
        ...
        "confidence_high_threshold": flat_config.get(
            "interpretability_confidence_high_threshold", 0.8
        ),
        "confidence_medium_threshold": flat_config.get(
            "interpretability_confidence_medium_threshold", 0.6
        ),
    }
