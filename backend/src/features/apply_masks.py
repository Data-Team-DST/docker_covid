...
class MaskApplicator:
    ...
    def get_statistics(self):
        ...
        for category in self.categories:
            ...
            print(
                f"{category:15} | Images: {num_images:4d} | Masques: {num_masks:4d} | Paires: {num_pairs:4d}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Appliquer des masques aux images COVID-19"
    )
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument(
        "--category",
        choices=["COVID", "Normal", "Lung_Opacity", "Viral Pneumonia", "all"],
        default="all",
        help="Catégorie à traiter",
    )
    parser.add_argument(
        "--method",
        choices=["overlay", "multiply", "extract"],
        default="overlay",
        help="Méthode d'application du masque",
    )
    parser.add_argument(
        "--output",
        default="data/processed/masked_images",
    ...
    if args.visualize:
        if args.category == "all":
            ...
            applicator.visualize_sample(
                args.category, num_samples=3, method=args.method
            )

    # Traiter les images
    if (
        not args.visualize
        or input("Continuer le traitement des images? (y/n): ").lower() == "y"
    ):
        if args.category == "all":
            for cat in applicator.categories:
