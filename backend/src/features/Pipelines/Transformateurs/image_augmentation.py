...
class ImageAugmenter(BaseEstimator, TransformerMixin):
    ...
    def _apply_brightness(self, img: np.ndarray) -> np.ndarray:
        ...
        if self.brightness_range is not None:
            ...
            img = np.clip(
                img * factor, 0, 255 if img.dtype == np.uint8 else 1.0
            )
        return img
    ...
    def transform(self, X: np.ndarray, y=None) -> np.ndarray:
        ...
        if self.verbose:
            logger.info(
                f"Augmenting {len(X)} images (p={self.probability})..."
            )
        ...
        iterator = (
            tqdm(data_array, desc="Augmentation")
            if self.verbose
            else data_array
        )
        ...
        for img in iterator:
            ...
            if self.rng_.random() < self.probability:
                ...
                if img_aug.shape != target_shape:
                    ...
                    zoom_factors = [
                        t / s for t, s in zip(target_shape, img_aug.shape)
                    ]
...
class ImageRandomCropper(BaseEstimator, TransformerMixin):
    ...
    def transform(self, X: np.ndarray, y=None) -> np.ndarray:
        ...
        if self.verbose:
            logger.info(
                f"Cropping completed: {n_cropped}/{len(X)} images cropped"
            )
            if cropped:
