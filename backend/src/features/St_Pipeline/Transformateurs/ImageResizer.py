...
class ImageResizer(BaseTransform):
    ...
    def _process(self, X: Any) -> Any:
                ...
                for idx, (_, row) in enumerate(X.iterrows()):
                    ...
                    if idx % 10 == 0 or idx == total - 1:
                        ...
                        self._update_progress(
                            progress, f"Redimensionné {idx + 1}/{total} images"
                        )
        ...
        # Cas 2: Liste d'images
            ...
            self._log(
                f"Redimensionnement de {len(X)} images en {self.img_size} (liste)"
            )
        ...
        # Cas 3: Numpy array (batch d'images)
            ...
            self._log(
                f"Redimensionnement de {X.shape[0]} images en {self.img_size} (numpy array)"
            )
    ...
    def visualize(
        ...
        fig.suptitle(
            f"ImageResizer: Redimensionnement en {self.img_size}",
            fontsize=16,
            fontweight="bold",
        )
