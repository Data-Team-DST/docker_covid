...
class ImagePCA(BaseTransform):
    ...
    def _fit(self, X: Any, y: Optional[Any] = None) -> None:
        ...
        self._log(
            f"Apprentissage PCA avec {self.n_components} composantes sur {X_flat.shape}"
        )
    ...
    def _plot_pca_variance(self):
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            with col1:
                ...
                fig1.add_hline(
                    y=0.95, line_dash="dash", line_color="red", annotation_text="95%"
                )
                fig1.add_hline(
                    y=0.90, line_dash="dash", line_color="orange", annotation_text="90%"
                )
                fig1.update_layout(
            ...
            st.info(
                f"📊 Total variance expliquée: **{cumsum[-1]:.2%}** avec {self.n_components} composantes"
            )
    ...
    def visualize(self, X_before: Any, X_after: Any, n_samples: int = 3) -> None:
        ...
        fig.suptitle(
            f"ImagePCA: Réduction à {self.n_components} composantes",
            fontsize=16,
            fontweight="bold",
        )
        ...
        axes[1].bar(
            range(1, min(20, len(self.pca.explained_variance_ratio_)) + 1),
        ...
        print(f"\n📊 Statistiques PCA:")
        print(
            f"   - Dimensions avant: {X_before.shape if hasattr(X_before, 'shape') else 'N/A'}"
        )
        ...
        if hasattr(X_before, "shape") and len(X_before.shape) == 2:
            ...
            print(
                f"   - Compression: {compression_ratio:.2%} ({X_before.shape[1]} → {X_after.shape[1]} features)"
            )
