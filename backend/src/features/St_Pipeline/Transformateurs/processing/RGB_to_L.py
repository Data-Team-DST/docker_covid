    ...
    def _process(self, X: Any) -> Any:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            if self.use_streamlit:
                self._plot_conversion_effect(
                    X["image_array"].tolist()[:6], gray_images[:6]
                )
        ...
        # Cas 2: Liste d'images
            ...
            gray_images = []
            for img in tqdm(
                X, desc=f"[{self.__class__.__name__}] RGB → L", disable=not self.verbose
            ):
                gray_images.append(self._convert_image(img))
            ...
            with col1:
                if len(images_before[0].shape) == 3:
                    ...
                    st.metric(
                        "Canaux avant",
                        "1 (déjà Grayscale)",
                        help="Image déjà en niveaux de gris",
                    )
            with col2:
                st.metric(
                    "Canaux après", "1 (Grayscale)", help="Image en niveaux de gris"
                )
