...
class ImageStandardScaler(BaseTransform):
    ...
    def _process(self, X: Any) -> Any:
        ...
        # Visualisation
        self._plot_standardization_effect(
            X_flat, X_scaled, X_before_stats, X_after_stats
        )
    ...
    def _plot_standardization_effect(
        self, X_before, X_after, stats_before, stats_after, n_samples=1000
    ):
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            fig.add_trace(
                go.Histogram(x=sample_before, nbinsx=50, name="Avant"), row=1, col=1
            )

            fig.add_trace(
                go.Histogram(x=sample_after, nbinsx=50, name="Après"), row=1, col=2
            )
            ...
            ax1.axvline(
                ...
                color="r",
                linestyle="--",
                label=f"Mean: {stats_before['mean']:.2f}",
            )
            ax1.legend()
            ...
            ax2.axvline(
                ...
                color="r",
                linestyle="--",
                label=f"Mean: {stats_after['mean']:.2f}",
            )
            ax2.legend()
