...
class TrainTestSplitter(BaseTransform):
        ...
        **kwargs,
        ...
        if val_size is not None:
            if test_size + val_size >= 1.0:
                raise ValueError(
                    f"test_size ({test_size}) + val_size ({val_size}) doit être < 1.0"
                )
    ...
    def _process(self, X: Any) -> dict:
        ...
        if self.label_column not in X.columns:
            raise ValueError(
                f"Colonne '{self.label_column}' introuvable dans le DataFrame"
            )
        ...
        if self.val_size is None:
            ...
            self._log(
                f"Split train/val/test: {train_pct:.0%}/{val_pct:.0%}/{test_pct:.0%}"
            )
            self._log(
    ...
    def _display_label_distribution(self, *label_arrays):
        ...
        fig, axes = plt.subplots(
            1, len(label_arrays), figsize=(6 * len(label_arrays), 4)
        )
        ...
        for idx, (name, labels) in enumerate(zip(split_names, label_arrays)):
            counts = labels.value_counts().sort_index()
            axes[idx].bar(
                range(len(counts)), counts.values, color="steelblue", alpha=0.7
            )
