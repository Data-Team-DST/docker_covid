...
class DatasetStatistics(BaseTransform):
    ...
    def _fit(self, X, y=None):
        ...
        self.stats_["class_percentages"] = (
            X["label"].value_counts(normalize=True) * 100
        ).to_dict()

        # Balance des classes
        counts = list(self.stats_["class_distribution"].values())
        self.stats_["class_balance_ratio"] = (
            max(counts) / min(counts) if min(counts) > 0 else float("inf")
        )

        # Masques
        if "mask_path" in X.columns:
            self.stats_["has_masks"] = X["mask_path"].notna().sum()
            self.stats_["mask_percentage"] = (
                self.stats_["has_masks"] / self.stats_["n_total"]
            ) * 100

        # Stats d'intensité pixel (optionnel)
        if self.compute_pixel_stats and "image_array" in X.columns:
            self._log(
                f"Calcul des stats d'intensité (échantillonnage {self.n_samples_per_class} par classe)..."
            )
            pixel_stats = {}
            ...
            for label in X["label"].unique():
                ...
                intensities = np.concatenate(
                    [
                        img.flatten()
                        for img in sample["image_array"].values
                        if img is not None
                    ]
                )
        ...
        self._log(
            f"Statistiques calculées: {self.stats_['n_total']} images, {len(self.stats_['class_distribution'])} classes"
        )
    ...
    def _plot_pixel_intensity_analysis(self):
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            ax.set_title(
                "Distribution Intensité Pixel par Classe",
                fontsize=14,
                fontweight="bold",
            )
