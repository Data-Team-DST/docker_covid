...
class ImageComparisonVisualizer(BaseTransform):
    ...
    def _process(self, X):
        ...
        if self.X_before_ is None:
            self._log(
                "Aucune donnée 'before' stockée, fit() requis d'abord", level="warning"
            )
            return X
    ...
    def _plot_difference_streamlit(self, imgs_before, imgs_after, n):
        ...
        fig.update_yaxes(showticklabels=False)
        fig.update_layout(
            height=600, showlegend=False, title_text="Comparaison avec Différence"
        )
    ...
    def _plot_side_by_side_matplotlib(self, imgs_before, imgs_after, n):
        ...
        for i in range(n):
            ...
            # Avant
            axes[0, i].imshow(
                img_before, cmap="gray" if len(img_before.shape) == 2 else None
            )
            ...
            # Après
            axes[1, i].imshow(
                img_after, cmap="gray" if len(img_after.shape) == 2 else None
            )
