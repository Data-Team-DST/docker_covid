...
class ImageHistogram(BaseTransform):
    ...
    def _process(self, X: Any) -> Any:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            self._log(
                f"Calcul des histogrammes ({self.bins} bins) pour {len(X)} images"
            )
            ...
            if self.use_streamlit and self._progress_bar is not None:
                ...
                for idx, (_, row) in enumerate(X.iterrows()):
                    ...
                    if img is not None:
                        histo = np.histogram(
                            img.flatten(), bins=self.bins, range=(0, 1)
                        )[0]
                        histos.append(histo)
                    else:
                    ...
                    if idx % 100 == 0 or idx == total - 1:
                        ...
                        self._update_progress(
                            progress, f"Histogramme {idx + 1}/{total}"
                        )
            ...
            if self.use_streamlit:
                self._plot_histogram_features(
                    X["image_array"].tolist()[:4], histos_array[:4]
                )
    ...
    def _plot_histogram_features(self, imgs, histos, n=4):
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            fig = make_subplots(
                ...
                row_heights=[0.5, 0.5],
                subplot_titles=[
                    f"Image {i+1}" if i < n else f"Histogramme ({self.bins} bins)"
                    for i in range(2 * n)
                ],
                specs=[[{"type": "image"}] * n, [{"type": "bar"}] * n],
            ...
            fig.update_layout(
                height=500, title_text=f"Features Histogrammes ({self.bins} bins)"
            )

            st.plotly_chart(fig, use_container_width=True)
            st.info(
                f"📦 Compression: {imgs[0].size} pixels → {self.bins} features par image"
            )
    ...
    def visualize(self, X_before: Any, X_after: Any, n_samples: int = 3) -> None:
        ...
        fig.suptitle(
            f"ImageHistogram: Extraction de features ({self.bins} bins)",
            fontsize=16,
            fontweight="bold",
        )
        ...
        for i in range(n_samples):
            ...
            # Histogramme extrait
            axes[1, i].bar(
                range(self.bins), histograms[i], color="steelblue", alpha=0.7
            )
