...
def generate_report(output_dir: Path, output_file: Path):
    ...
    report.append("")
    report.append(
        "This report presents a comprehensive exploratory data analysis of the COVID-19 radiography dataset, "
    ...
    if not df_stats.empty:
        ...
        report.append("")
        report.append(
            f"- **Average Width**: {df_stats['width'].mean():.1f} px (±{df_stats['width'].std():.1f})"
        )
        report.append(
            f"- **Average Height**: {df_stats['height'].mean():.1f} px (±{df_stats['height'].std():.1f})"
        )
        report.append(
            f"- **Width Range**: {df_stats['width'].min():.0f} - {df_stats['width'].max():.0f} px"
        )
        report.append(
            f"- **Height Range**: {df_stats['height'].min():.0f} - {df_stats['height'].max():.0f} px"
        )
        report.append("")
        ...
        report.append("")
        report.append(
            f"- **Average Mean Intensity**: {df_stats['mean'].mean():.2f} (±{df_stats['mean'].std():.2f})"
        )
        report.append(
            f"- **Average Std Intensity**: {df_stats['std'].mean():.2f} (±{df_stats['std'].std():.2f})"
        )
        report.append("")
        ...
        report.append("")
        report.append(
            "| Class | Avg Width | Avg Height | Avg Mean Intensity | Avg Std Intensity |"
        )
        report.append(
            "|-------|-----------|------------|-------------------|-------------------|"
        )
        for cls in sorted(df_stats["class"].unique()):
            ...
            report.append(
                f"| {cls} | {cls_df['width'].mean():.1f} | {cls_df['height'].mean():.1f} | "
    ...
    if pca_path.exists():
        ...
        report.append(
            f"- **Variance explained by top 10**: {df_pca['cumulative_variance'].iloc[9]:.2%}"
        )
        report.append("")
    ...
    if summary:
        ...
        if corrupted > 0:
            ...
            observations.append(
                f"- **Data Quality**: Found {corrupted} corrupted images ({pct:.2f}% of dataset)"
            )
        else:
            observations.append(
                "- **Data Quality**: All images loaded successfully, no corrupted files detected"
            )
    ...
    if not df_stats.empty:
        ...
        if imbalance_ratio > 2:
            observations.append(
                f"- **Class Imbalance**: Dataset shows class imbalance (ratio: {imbalance_ratio:.2f}:1)"
            )
        else:
            observations.append(
                "- **Class Balance**: Dataset is relatively well-balanced across classes"
            )
    ...
    if summary and "kmeans_metrics" in summary:
        ...
        if ari > 0.5:
            observations.append(
                f"- **Clustering**: Strong clustering performance (ARI={ari:.3f}), suggesting distinct visual patterns per class"
            )
        elif ari > 0.3:
            observations.append(
                f"- **Clustering**: Moderate clustering performance (ARI={ari:.3f}), some overlap between classes"
            )
        else:
            observations.append(
                f"- **Clustering**: Weak clustering performance (ARI={ari:.3f}), significant overlap between classes"
            )
    ...
    if pca_path.exists():
        ...
        if var_10 > 0.8:
            observations.append(
                f"- **Dimensionality**: High variance captured by top 10 PCs ({var_10:.2%}), suggesting strong latent structure"
            )
        else:
            observations.append(
                f"- **Dimensionality**: Moderate variance by top 10 PCs ({var_10:.2%}), complex feature space"
            )
    ...
    report.append("")
    report.append(
        "1. **Feature Engineering**: Explore additional features from masked regions"
    )
    report.append(
        "2. **Data Augmentation**: Consider augmentation strategies to balance classes"
    )
    report.append(
        "3. **Deep Learning**: Train CNN classifiers using the extracted embeddings"
    )
    report.append(
        "4. **Attention Mechanisms**: Investigate Grad-CAM to understand model focus"
    )
    report.append(
        "5. **Cross-validation**: Validate clustering results with k-fold cross-validation"
    )
    report.append(
        "6. **Ensemble Methods**: Combine multiple clustering algorithms for robust results"
    )
    report.append("")
    ...
    report.append("")
    report.append(
        "This comprehensive EDA provides valuable insights into the COVID-19 radiography dataset. "
