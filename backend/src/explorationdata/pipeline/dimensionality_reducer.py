...
class DimensionalityReducer:
    ...
    def plot_pca_scree(self, output_path: Path, max_components: int = 50):
    ...
    def save_pca_components(self, output_path: Path, top_n: int = 10):
        ...
        self.logger.info(f"Saved top {top_n} PCA components to {output_path}")
    ...
    def fit_tsne(
        ...
        """
        self.logger.info(f"Fitting t-SNE (perplexity={perplexity})...")

        # Sample if dataset is too large
        sample_indices = None
        if max_samples and len(embeddings) > max_samples:
            self.logger.info(f"Sampling {max_samples} points for t-SNE")
