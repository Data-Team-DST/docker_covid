...
class Visualizer:
    ...
    def plot_class_distribution(self, df: pd.DataFrame, output_path: Path):
    ...
    def plot_image_dimensions(self, df: pd.DataFrame, output_path: Path):
        ...
        axes[0, 0].axvline(
            df["width"].median(), color="red", linestyle="--", label="Median"
        )
        axes[0, 0].legend()
        ...
        axes[0, 1].axvline(
            df["height"].median(), color="red", linestyle="--", label="Median"
        )
        axes[0, 1].legend()
    ...
    def plot_intensity_statistics(self, df: pd.DataFrame, output_path: Path):
        ...
        for cls in df["class"].unique():
            ...
            axes[1, 0].scatter(
                cls_df["mean"], cls_df["std"], alpha=0.5, label=cls, s=10
            )
    ...
    def create_image_grid(
        ...
        fig, axes = plt.subplots(
            n_classes, n_per_class, figsize=(n_per_class * 2, n_classes * 2)
        )
    ...
    def create_image_mask_overlay(
        self, df: pd.DataFrame, output_path: Path, n_samples: int = 8
    ):
        ...
        # Sample images
        sampled = df_masked.sample(
            n=min(n_samples, len(df_masked)), random_state=self.random_state
        )
