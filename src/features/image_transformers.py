...
class ImageResizer(BaseTransform):
    ...
    def visualize(
        ...
        fig.suptitle(
            f"ImageResizer: Resizing to {self.img_size}",
            fontsize=16,
            fontweight="bold",
        )
...
class ImageAugmenter(BaseTransform):
    ...
    def _process(self, X: Any) -> Any:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            if self.verbose:
                self._log(
                    f"Augmentation complete: {n_augmented}/{len(X)} images augmented"
                )
        ...
        # Case 2: List or numpy array
            ...
            if self.verbose:
                self._log(
                    f"Augmentation complete: {n_augmented}/{len(data_array)} images augmented"
                )
    ...
    def visualize(
        ...
        fig, axes = plt.subplots(2, n_samples, figsize=(4 * n_samples, 8))
        fig.suptitle(
            f"ImageAugmenter: Random augmentations (p={self.probability})",
            fontsize=16,
            fontweight="bold",
        )
        ...
        print(
            f"   - Images augmented: {self.n_images_augmented_}/{len(images_before) if images_before else 0}"
        )
...
class ImageNormalizer(BaseTransform):
    ...
    def visualize(
        ...
        fig.suptitle(
            "ImageNormalizer: 0-1 Normalization",
            fontsize=16,
            fontweight="bold",
        )
        ...
        for i in range(n_samples):
            ...
            axes[0, i].set_title(
                f"Before: [{images_before[i].min():.3f}, {images_before[i].max():.3f}]"
            )
            axes[0, i].axis("off")
            ...
            axes[1, i].set_title(
                f"After: [{images_after[i].min():.3f}, {images_after[i].max():.3f}]"
            )
            axes[1, i].axis("off")

            # Histograms
            axes[2, i].hist(
                images_before[i].ravel(),
                bins=50,
                alpha=0.5,
                label="Before",
                color="blue",
...
class ImageRandomCropper(BaseTransform):
    ...
    def _crop_image(self, img: np.ndarray) -> np.ndarray:
        ...
        if h < ch or w < cw:
            self._log(
                f"Image too small ({h}x{w}) for crop ({ch}x{cw}), skipped",
                level="warning",
            )
            return img
