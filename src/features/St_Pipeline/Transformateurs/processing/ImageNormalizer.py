...
class ImageNormalizer(BaseTransform):
    ...
    def _process(self, X: Any) -> Any:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            if self.use_streamlit:
                self._plot_normalization_effect(
                    X["image_array"].tolist()[:4], norm_images[:4]
                )
    ...
    def _plot_normalization_effect(self, imgs_before, imgs_after, n=4):
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            fig = make_subplots(
                ...
                subplot_titles=[
                    (
                        f"Avant: [{imgs_before[i].min():.1f}, {imgs_before[i].max():.1f}]"
                        if i < n
                        else f"Après: [0.0, 1.0]" if i < 2 * n else "Distribution"
                    )
                    for i in range(3 * n)
                ],
                specs=[
            ...
            fig.update_layout(
                height=700, title_text="Normalisation [0, 1]", barmode="overlay"
            )
    ...
    def visualize(self, X_before: Any, X_after: Any, n_samples: int = 3) -> None:
        ...
        fig.suptitle(
            "ImageNormalizer: Normalisation 0-1", fontsize=16, fontweight="bold"
        )
        ...
        for i in range(n_samples):
            ...
            axes[0, i].set_title(
                f"Avant: [{images_before[i].min():.3f}, {images_before[i].max():.3f}]"
            )
            axes[0, i].axis("off")
            ...
            axes[1, i].set_title(
                f"Après: [{images_after[i].min():.3f}, {images_after[i].max():.3f}]"
            )
            axes[1, i].axis("off")
        ...
        print(
            f"   - Avant: [{images_before[0].min():.3f}, {images_before[0].max():.3f}]"
        )
