...
class ImageAugmenter(BaseTransform):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if brightness_range is not None and not isinstance(
            brightness_range, (tuple, list)
        ):
    ...
    def _process(self, X: Any) -> Any:
        ...
        if isinstance(X, pd.DataFrame):
            ...
            self._log(
                f"Augmentation de {len(X)} images (p={self.probability}) (DataFrame)"
            )
            ...
            if self.verbose:
                self._log(
                    f"Augmentation terminée: {n_augmented}/{len(X)} images augmentées"
                )
        ...
        # Cas 2: Liste ou numpy array
            ...
            self._log(
                f"Augmentation de {len(data_array)} images (p={self.probability})"
            )
            ...
            if self.verbose:
                self._log(
                    f"Augmentation terminée: {n_augmented}/{len(data_array)} images augmentées"
                )
        ...
        if self.zoom_range is not None:
            ...
            if zoom_factor != 1.0:
                ...
                img_zoomed = cv2.resize(
                    img_aug, (new_w, new_h), interpolation=cv2.INTER_LINEAR
                )
    ...
    def _plot_augmentation_comparison(self, X_before, X_after, n_samples=6):
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            fig.update_layout(
                height=400, showlegend=False, title_text="Avant (haut) vs Après (bas)"
            )
            st.plotly_chart(fig, use_container_width=True)

            st.info(
                f"Augmenté: {self.n_images_augmented_}/{len(X_before)} images (p={self.probability})"
            )
    ...
    def visualize(self, X_before: Any, X_after: Any, n_samples: int = 3) -> None:
        ...
        if isinstance(X_before, pd.DataFrame):
            images_before = [
                X_before["image_array"].iloc[i]
                for i in range(min(n_samples, len(X_before)))
            ]
            images_after = [
                X_after["image_array"].iloc[i]
                for i in range(min(n_samples, len(X_after)))
            ]
        else:
        ...
        fig.suptitle(
            f"ImageAugmenter: Augmentations aléatoires (p={self.probability})",
            fontsize=16,
            fontweight="bold",
        )
        ...
        for i in range(n_samples):
            ...
            axes[0, i].imshow(
                images_before[i],
                cmap="gray" if len(images_before[i].shape) == 2 else None,
            )
            ...
            axes[1, i].imshow(
                images_after[i],
                cmap="gray" if len(images_after[i].shape) == 2 else None,
            )
        ...
        print(
            f"   - Images augmentées: {self.n_images_augmented_}/{len(images_before) if images_before else 0}"
        )
