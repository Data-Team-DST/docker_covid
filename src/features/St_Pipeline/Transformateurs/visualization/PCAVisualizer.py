...
class PCAVisualizer(BaseTransform):
    ...
    def _plot_variance_explained(self, X_pca):
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            fig = make_subplots(
                rows=1, cols=2, subplot_titles=("Scree Plot", "Variance Cumulée")
            )
    ...
    def _plot_projection_2d(self, X_pca, y):
        ...
        if X_pca.shape[1] < 2:
            self._log(
                "Besoin d'au moins 2 composantes pour projection 2D", level="warning"
            )
            return
        ...
        if self.use_streamlit and HAS_STREAMLIT:
            ...
            df = pd.DataFrame({"PC1": X_pca[:, 0], "PC2": X_pca[:, 1], "Classe": y})
    ...
    def _plot_projection_3d(self, X_pca, y):
        ...
        if X_pca.shape[1] < 3:
            self._log(
                "Besoin d'au moins 3 composantes pour projection 3D", level="warning"
            )
            return
