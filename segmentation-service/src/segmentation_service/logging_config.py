"""Configuration logging — DS_COVID Segmentation Service.

Logs JSON structurés sur stdout + fichier rotatif tmp/logs/segmentation-service.log
(si accessible) + envoi asynchrone non-bloquant au log-service central (si
disponible).

Dupliqué depuis data-service/src/data_service/logging_config.py plutôt
qu'importé : chaque service reste buildable/déployable indépendamment,
conformément aux frontières de service du projet (cf. US-23).
"""
...
def setup_logging() -> None:
    """Configure le logging segmentation-service."""
    _configure_root(service_name="segmentation-service")
