...
## Module Naming Convention (FastAPI services)
...
### Review checklist — file naming

When reviewing a Python PR, check:
- [ ] New `.py` files follow the suffix convention above
- [ ] `*_router.py` files contain **only** endpoint handlers and Pydantic schemas
- [ ] Business logic is isolated in `*_service.py`
- [ ] No file exceeds its type limit (run `wc -l` on modified files)

### Quick audit command

```bash
# Flag any router with private helper functions (signal of mixed responsibility)
grep -n "^def _\|^async def _" src/**/*_router.py 2>/dev/null

# Flag files over their type limit
python3 - <<'EOF'
