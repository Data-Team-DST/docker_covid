...
> Trimmed 2026-08-24 (skill-stocktake) to only the skills/agents actually installed in this
> project's `.claude/` — the original table referenced ~20 that don't exist here and read as
> instructions to reach for tooling that was never installed.

| SWE surface | MLE use |
|-------------|---------|
| `python-testing` | Test feature transforms, split logic, metric calculations, artifact loading, and inference schemas before implementation |
| `mle-reviewer` | Review code quality plus ML-specific leakage, reproducibility, promotion, and monitoring risks |
| `eval-harness` | Turn offline metrics, slice checks, latency budgets, and rollback drills into repeatable gates |
| `database-migrations` / `postgres-patterns` | Version labels, feature snapshots, prediction logs, experiment metrics, and drift analytics |
...
| `python-patterns` | Follow Pythonic idioms and type hints for transforms, scoring code, and pipeline utilities |
...
> "Streamlined ECC path" column removed 2026-08-24 (skill-stocktake) — every row referenced
> mostly-phantom skills/agents not installed in this project. The task/output/lanes columns
> are the durable, tool-agnostic part; for the SWE-surface pairing, use the trimmed table
> above instead of inventing a path per row.

| ID | Common MLE task | Required output | Pipeline lanes covered |
|----|-----------------|-----------------|------------------------|
