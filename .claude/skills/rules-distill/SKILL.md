...
Check `jq` is available first: `command -v jq`. Both scripts below depend on it (confirmed:
`jq: command not found` on a jq-less Windows/Git-Bash machine breaks both outright). If `jq`
is missing, skip to the manual fallback rather than running them and hitting the error.
...
**Manual fallback (no `jq`)**: enumerate skills/rules directly with Glob + Read (same
approach as skill-stocktake's own jq-less fallback — reuse its inventory output directly if
it was run first, since Phase 0 of this skill's typical trigger is "after a skill-stocktake
reveals patterns"). Confirmed viable in practice.
